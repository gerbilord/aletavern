...
export default class gameEngine {
    // TODO The otherjoingame message is sent right away.
    ...
    sendPrompts() {
        ...
        this.stateData = {
            ...
            numOfAnswersNeeded: this.players.length,
        };
        this.answers = [];
        ...
        var onDone = (msg) => {
            if (
                ...
                this.answers.length >= this.players.length
            ) {
                this.ws.sendMessageToAll({ type: 'please wait' });
                this.ws.onMessageGame = [];
        ...
        var onAnswer = (msg) => {
            if (msg.data.type == 'answer') {
                ...
                this.stateData.numOfAnswersNeeded =
                    this.stateData.numOfAnswersNeeded - 1;
                console.log('Answer!');
            }
        };
    ...
    async sendAllBallots() {
        ...
        for (var i; i < this.answers.length; i = i + 2) {
            // TODO make variable
            await this.sendBallot(this.answers.slice(i, i + 1)); // TODO makes this varaible
        }
    }

    sendBallot(listOfAnswersToVoteOn) {
        // TODO, include id's of players who prompt it is?
        // Players don't vote on their own answers/questions
        var numPlayersYetToVote =
            this.players.length - listOfAnswersToVoteOn.length;
        var playersWhoVoted = [];

        var promise = new Promise((resolve, reject) => {
            let voteListener = (msgObj) => {
                // TODO only let each player vote once;
