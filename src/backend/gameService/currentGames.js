...
function getPlayersInGame(gameId) {
    ...
    if (game != null) {
        ...
        console.debug(
            'Requested players from invalid game. Bad game id: ' + gameId
        );
        return [];
    }
}
...
function getHostOfGame(gameId) {
    // TODO if game has no host, delete it.
    var game = currentGames[gameId];
    ...
    if (game != null) {
        ...
        console.debug(
            'Requested host from invalid game. Bad game id: ' + gameId
        );
...
function getWsId(playerWs) {
    // TODO Clean up map once in a while // This works but could probably be improved
    if (wsToPlayerId.has(playerWs)) {
        // && wsToPlayerId.get(playerWs) != undefined) // If a websocket is trying to join multiple games, clean up previous game.
        console.log(
            'Game joined/created before leaving previous one! Culprit:' +
                wsToPlayerId.get(playerWs)
        );
        ...
        if (player) {
            ...
            console.log(
                'Something is FATALLY WRONG with the logic. Culprit:' +
                    wsToPlayerId.get(playerWs)
            );
        }
    }
...
function makeGameId() {
    ...
    while (currentGames[result] != null) {
        // TODO if game is old just replace it
...
class Game {
    ...
    getPlayersByWs(ws) {
        var matchingPlayers = this.players.filter((player) => {
            return player.ws == ws;
        });
    ...
    replacePlayerWs(oldWs, newWs) {
        this.getPlayersByWs(oldWs).forEach((player) => {
            player.ws = newWs;
        });
    }
}
